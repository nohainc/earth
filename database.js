  ...
  return {
    ...
    async loadCanonical() {
      ...
      const [
        ...
        safeQuery('select id, institution_id, title, body, status, quorum, approval_threshold, implementation_delay_days, outcome, execution_status, correlation_id, rule_version_id, opens_game_day, opens_game_minute, closes_game_day, closes_game_minute, implementation_game_day, implementation_game_minute, target_category, target_value_json, target_kind, building_catalog_id, research_project_id, extract(epoch from opens_at)*1000 as opens_at, extract(epoch from closes_at)*1000 as closes_at, extract(epoch from implementation_at)*1000 as implementation_at, extract(epoch from resolved_at)*1000 as resolved_at, extract(epoch from executed_at)*1000 as executed_at, extract(epoch from created_at)*1000 as created_at from proposals order by created_at desc limit 100'),
