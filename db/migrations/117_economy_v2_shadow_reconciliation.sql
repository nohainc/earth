-- EARTH ACTIVE MIGRATION: auditable Economy V2 shadow comparison storage.
-- The comparison is observational only; it never becomes an economic authority.
...
CREATE TABLE IF NOT EXISTS economy_shadow_runs (
  ...
  status TEXT NOT NULL CHECK (status IN ('captured', 'reconciled', 'skipped', 'failed')),
