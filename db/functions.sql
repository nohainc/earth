...
-- Economy V2 foundation: compact owner identifiers are immutable once issued.
...
-- Economy V2 idempotency primitive. The insert is attempted first; retries
-- resolve the existing row only after the unique correlation conflict.
...
RETURNS TABLE (transaction_id BIGINT, correlation_id TEXT, created BOOLEAN)
LANGUAGE plpgsql
AS $$
DECLARE existing economic_transactions%ROWTYPE;
BEGIN
  ...
  RETURNING id, economic_transactions.correlation_id INTO transaction_id, correlation_id;
  IF transaction_id IS NOT NULL THEN created := TRUE; RETURN NEXT; RETURN; END IF;
  SELECT * INTO existing FROM economic_transactions WHERE economic_transactions.correlation_id = p_correlation_id;
  IF existing.game_day <> p_game_day OR existing.game_minute <> p_game_minute
     OR existing.transaction_kind <> p_transaction_kind OR existing.source_type <> p_source_type
     OR existing.source_id IS DISTINCT FROM p_source_id OR existing.rules_version <> p_rules_version THEN
    RAISE EXCEPTION 'Correlation ID % was already used with different transaction parameters', p_correlation_id;
  END IF;
  transaction_id := existing.id; correlation_id := existing.correlation_id; created := FALSE;
  RETURN NEXT;
END;
$$;
