...
const restoreArgs = ['--no-owner', '--no-privileges', '--exclude-schema=pscale_extensions', '--dbname', target, dumpPath];
if (process.env.EARTH_RESTORE_CLEAN === 'true') restoreArgs.unshift('--clean', '--if-exists');
await run('pg_restore', restoreArgs, { maxBuffer: 1024 * 1024 });
