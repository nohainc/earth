...
void main() {
  ...
  group('NavigationDeepLink', () {
    test('stub defaults return null and allow section updates without throw', () {
      ...
      NavigationDeepLink.updateSection('map');
      NavigationDeepLink.listen((_) {});
    });
  });

  group('Complex Systems Page Mode & Dashboard Full Pages', () {
    testWidgets('PlanetaryMapDialog renders in page mode and triggers return to command', (tester) async {
    ...
    testWidgets('Dashboard renders map section as full page and title', (tester) async {
      tester.view.physicalSize = const Size(1280, 900);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);

      expect(dashboardSectionTitle('map'), equals('PLANETARY GRID'));
      expect(dashboardSectionTitle('dynasty'), equals('DYNASTY TREE'));
      expect(dashboardSectionTitle('derivatives'), equals('FUTURES & CHARTS'));
      expect(dashboardSectionTitle('net_worth'), equals('NET WORTH ANALYTICS'));
      expect(dashboardSectionTitle('contracts'), equals('CONTRACTS'));
      ...
      expect(find.byType(PlanetaryMapDialog), findsOneWidget);
    });
