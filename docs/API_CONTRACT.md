...
- **Internal State & Events**: Event details (`world_events.details`), negotiated contract terms (`terms_markup`), governance rule values (`value_json`), and governance/constitutional rule payloads (`charter_rules`) are canonically encoded using **Nano Markup** (`nanomarkup` by `nohainc`).
