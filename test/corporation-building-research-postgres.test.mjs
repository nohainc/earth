...
test('private-tier research casts the tier parameter before using it in catalog arithmetic', async () => {
  ...
  const client = {
    async query(sql, params = []) {
      ...
      if (normalized.includes('FROM building_catalog WHERE id = $1 AND building_type = $2')) return { rows: [{ id: 'private-estate-plot-t2', tier: 2, cost_credits: '1700', construction_days: 2, slot_footprint: 2, ownership_class: 'private' }], rowCount: 1 };
