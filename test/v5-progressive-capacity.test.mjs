...
test('V4 operational actions use the shared proposal action registry', async () => {
  ...
  for (const action of ['PUBLIC_PROJECT', 'RESEARCH_FUNDING', 'ORGANIZATION_TECHNOLOGY_ADOPTION']) {
...
test('V5 cutover readiness is fail-closed and read-only', async () => {
  ...
  assert.match(service, /noActiveLegacyConstitutionalProposals/);
  assert.match(service, /governance_proposals_v4/);
  assert.match(service, /active_legacy_constitutional_proposals/);
...
test('V5 client finance projections consume the authoritative server ledger', async () => {
  ...
  assert.match(finance, /nextSettlement\['inflowsUnits'\]/);
  assert.match(finance, /_forecastCategoryUnits\(nextSettlement, 'TAX'\)/);
