...
test('service capacity is allocated once across Houses in deterministic order', async () => {
  ...
  const repository = {
    async query(sql) {
      ...
      if (query.includes('from need_rules')) return { rows: [{ need_code: 'HEALTH', service_type_code: 'HEALTH', demand_units_per_human: '1', critical_threshold_bps: 7500, rules_version: 'needs-v1' }] };
      ...
      if (query.includes('from service_types')) return { rows: [{ code: 'HEALTH', daily_price_units: '0' }] };
      if (query.includes('from buildings b')) return { rows: [{ territory_id: 'T-1', service_code: 'HEALTH', economic_id: 'PROVIDER-1', owner_type: 'CORPORATION', capacity_units: '1' }] };
