...
export async function settleLifeMaintenanceInTransaction(tx: PostgresRepository, day: number): Promise<number> {
  await tx.query(
    "INSERT INTO account_balances (account_id, owner_id, currency, balance) VALUES ('account-ouc-treasury', 'OUC', 'CREDIT', 0.00) ON CONFLICT (account_id) DO NOTHING",
  );
