...
export async function castVote(repository: PostgresRepository, input: { proposalId: string; humanId: string; choice: string }): Promise<Record<string, unknown>> {
  return repository.transaction(async (tx) => {
    ...
    try {
      const ballot = await tx.query('INSERT INTO ballots (proposal_id, human_id, choice, weight) VALUES ($1,$2,$3,$4) ON CONFLICT (proposal_id, human_id) DO NOTHING', [input.proposalId, input.humanId, input.choice, weight]);
