...
export async function createProposal(repository: PostgresRepository, input: { humanId: string; institutionId: string; title: string; body: string; durationHours: number; ruleVersionId?: string; targetCategory: string | null; targetValue: Record<string, unknown> | null; correlationId: string }): Promise<Record<string, unknown>> {
  return repository.transaction(async (tx) => {
    ...
    await tx.query("INSERT INTO proposals (id, institution_id, title, body, status, opens_at, closes_at, rule_version_id, quorum, approval_threshold, implementation_delay_days, implementation_at, target_category, target_value_json, correlation_id) VALUES ($1,$2,$3,$4,'open',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP + ($5 * INTERVAL '1 hour'),$6,$7,$8,$9,CURRENT_TIMESTAMP + (($5 + $9 * 24) * INTERVAL '1 hour'),$10,$11,$12)", [proposalId, input.institutionId, input.title, input.body, input.durationHours, rule.rows[0].id, quorum, approvalThreshold, implementationDelay, input.targetCategory, input.targetValue ? JSON.stringify(input.targetValue) : null, input.correlationId]);
...
export async function castVote(repository: PostgresRepository, input: { proposalId: string; humanId: string; choice: string }): Promise<Record<string, unknown>> {
  return repository.transaction(async (tx) => {
    const proposal = await tx.query<{ institution_id: string }>("SELECT institution_id FROM proposals WHERE id = $1 AND status = 'open' AND closes_at > CURRENT_TIMESTAMP", [input.proposalId]);
...
export async function resolveProposals(repository: PostgresRepository): Promise<number> {
  const proposals = await repository.query<{ id: string; quorum: string; approval_threshold: string }>("SELECT id, quorum, approval_threshold FROM proposals WHERE status = 'closed' AND outcome = 'pending'");
  let resolved = 0;
...
export async function executeProposal(repository: PostgresRepository, input: { proposalId: string; humanId: string }): Promise<Record<string, unknown>> {
  return repository.transaction(async (tx) => {
    const proposal = await tx.query<{ id: string; institution_id: string; title: string; outcome: string; executed_at: string | null; implementation_at: string | null; target_category: string | null; target_value_json: unknown; execution_status: string }>('SELECT * FROM proposals WHERE id = $1 FOR UPDATE', [input.proposalId]);
    ...
    if (current.implementation_at && new Date(current.implementation_at).getTime() > Date.now()) throw new Error('Implementation delay has not elapsed');
