...
export async function castVote(repository: PostgresRepository, input: { proposalId: string; humanId: string; choice: string }): Promise<Record<string, unknown>> {
  return repository.transaction(async (tx) => {
    ...
    const senateGavel = await tx.query("SELECT 1 FROM dynasty_heirlooms WHERE heirloom_type = 'senate_gavel' AND equipped_by_human_id = $1 LIMIT 1", [input.humanId]);
    const dynastyInfluence = (diplomaticPerk.rows[0] ? 1.15 : 1) * (senateGavel.rows[0] ? 1.1 : 1);
