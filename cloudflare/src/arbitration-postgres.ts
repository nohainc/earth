...
export async function resolveContractDispute(
  ...
  return repository.transaction(async (tx) => {
    ...
    const contract = await tx.query<{ id: string; proposer_id: string; counterparty_id: string; amount: string; terms_markup: string | null; proposer_business_id: string | null; counterparty_business_id: string | null }>('SELECT id, proposer_id, counterparty_id, amount, terms_markup, proposer_business_id, counterparty_business_id FROM negotiated_contracts WHERE id = $1 FOR UPDATE', [input.contractId]);
    ...
    if (input.outcome === 'void') {
      ...
      const terms = fromNanoMarkup<Record<string, unknown>>(contract.rows[0].terms_markup ?? '');
      const proposerBusinessId = contract.rows[0].proposer_business_id ?? (typeof terms.proposerBusinessId === 'string' ? terms.proposerBusinessId : null);
      const counterpartyBusinessId = contract.rows[0].counterparty_business_id ?? (typeof terms.counterpartyBusinessId === 'string' ? terms.counterpartyBusinessId : null);
