...
async function foundingPolicy(tx: PostgresRepository, day: number): Promise<FoundingPolicy> {
  const row = (await tx.query<{ id: string; version: number; founding_fee_units: string; initial_treasury_reserve_units: string; initial_house_base_capacity_rate_units: string; rules_version: string }>(`SELECT id, version, founding_fee_units::TEXT, initial_treasury_reserve_units::TEXT, initial_house_base_capacity_rate_units::TEXT, rules_version
  ...
  return { id: row.id, version: Number(row.version), fee: BigInt(row.founding_fee_units), reserve: BigInt(row.initial_treasury_reserve_units), initialHouseRate: BigInt(row.initial_house_base_capacity_rate_units), rulesVersion: row.rules_version };
}
  ...
  return repository.transaction(async (tx) => {
    ...
    await tx.query(`INSERT INTO constitutional_rule_versions_v5
      ...
      VALUES ($1, 'CORPORATION.HOUSE_CAPACITY.BASE_RATE', 'CORPORATION', $2, 1,
              $3::JSONB, $4, 'ACTIVE')`,
      [`V5-CONST-CORP-HOUSE-RATE-${id}-1`, id, JSON.stringify({ value: policy.initialHouseRate.toString() }), day]);
    ...
    if (requiredFounderFunds > 0n) {
      ...
      await tx.query(`SELECT earth_post_transaction($1,$2,1439,'ASSET_TRANSFER','CORPORATION_FOUNDING_FEE',$3,$4,$5::JSONB)`, [input.correlationId, day, id, policy.rulesVersion, JSON.stringify(entries)]);
    }
