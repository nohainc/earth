...
async function getClock(repo: PostgresRepository) {
  ...
  if (!result.rows[0]) throw new Error('World clock is not configured');
...
async function assertActor(repo: PostgresRepository, houseId: string, humanId: string) {
  ...
  if (!result.rows[0]) throw new Error('Active Human does not belong to this House');
}
...
async function loadCommunity(repo: PostgresRepository, communityId: string, lock = false) {
  ...
  if (!result.rows[0]) throw new Error('Community not found');
...
function requireManager(role: CommunityRole | null) {
  if (role !== 'OWNER' && role !== 'MODERATOR') throw new Error('Community owner or moderator permission required');
}
...
export async function listCommunities(repo: PostgresRepository, houseId?: string, membership: 'mine' | undefined = undefined) {
  const result = await repo.query(`SELECT c.*, (SELECT COUNT(*)::int FROM community_memberships all_cm WHERE all_cm.community_id=c.id AND all_cm.status='ACTIVE') AS member_count, (SELECT h.house_name FROM houses h WHERE h.id=c.founder_house_id) AS founder_house_name, cm.role AS viewer_role, cm.status AS viewer_membership_status FROM communities c LEFT JOIN community_memberships cm ON cm.community_id=c.id AND cm.house_id=$1 WHERE c.status='ACTIVE' ${membership === 'mine' ? "AND cm.status='ACTIVE'" : ''} ORDER BY c.created_at DESC`, [houseId ?? null]);
  return { ok: true, communities: result.rows };
}

export async function getCommunity(repo: PostgresRepository, communityId: string, houseId: string) {
  const result = await loadCommunity(repo, communityId);
  if (result.visibility === 'PRIVATE' && !(await getRole(repo, communityId, houseId))) throw new Error('Community membership required');
  return { ok: true, community: result };
}
...
export async function createCommunity(repo: PostgresRepository, input: { houseId: string; humanId: string; name: string; description?: string; visibility?: string; joinPolicy?: string; correlationId: string }) {
  return repo.transaction(async (tx) => {
    ...
    if (name.length < 3 || name.length > 80) throw new Error('Community name must be 3–80 characters');
    if (description.length > 5000) throw new Error('Community description is too long');
    const visibility = input.visibility ?? 'PUBLIC'; const joinPolicy = input.joinPolicy ?? 'OPEN';
    if (!['PUBLIC', 'PRIVATE'].includes(visibility) || !['OPEN', 'REQUEST'].includes(joinPolicy)) throw new Error('Invalid community rules');
...
export async function updateCommunity(repo: PostgresRepository, input: { communityId: string; houseId: string; humanId: string; name?: string; description?: string; visibility?: string; joinPolicy?: string }) {
  return repo.transaction(async (tx) => {
    ...
    if (input.name !== undefined) { const name = cleanName(input.name); if (name.length < 3 || name.length > 80) throw new Error('Community name must be 3–80 characters'); add('name', name); add('normalized_name', normalizedName(name)); }
    if (input.description !== undefined) { if (input.description.length > 5000) throw new Error('Community description is too long'); add('description', input.description.trim()); }
    if (input.visibility !== undefined) { if (!['PUBLIC', 'PRIVATE'].includes(input.visibility)) throw new Error('Invalid visibility'); add('visibility', input.visibility); }
    if (input.joinPolicy !== undefined) { if (!['OPEN', 'REQUEST'].includes(input.joinPolicy)) throw new Error('Invalid join policy'); add('join_policy', input.joinPolicy); }
    if (!fields.length) throw new Error('No community fields to update'); fields.push('updated_at=CURRENT_TIMESTAMP'); values.push(input.communityId);
...
export async function changeCommunityMembership(repo: PostgresRepository, input: { communityId: string; houseId: string; humanId: string; action: MembershipAction; targetHouseId?: string; applicationMessage?: string; correlationId?: string }) {
  return repo.transaction(async (tx) => {
    await assertActor(tx, input.houseId, input.humanId); const c = await loadCommunity(tx, input.communityId, true); if (c.status !== 'ACTIVE') throw new Error('Community is disbanded');
    if (input.action === 'remove') {
      requireManager(await getRole(tx, input.communityId, input.houseId, true));
      if (!input.targetHouseId || input.targetHouseId === input.houseId) throw new Error('A different member House is required');
      ...
      if (!target || target.status !== 'ACTIVE') throw new Error('Target House is not an active member');
      if (target.role === 'OWNER') throw new Error('Owners must be transferred or demoted before removal');
    ...
    if (input.action === 'leave') {
      if (!existing.rows[0] || existing.rows[0].status !== 'ACTIVE') throw new Error('House is not an active member');
      if (existing.rows[0].role === 'OWNER') { const owners = await tx.query(`SELECT house_id FROM community_memberships WHERE community_id=$1 AND role='OWNER' AND status='ACTIVE' FOR UPDATE`, [input.communityId]); if (owners.rows.length < 2) throw new Error('The last owner cannot leave'); }
    ...
    if (existing.rows[0]?.status === 'ACTIVE') throw new Error('House is already an active member');
...
export async function listCommunityMembers(repo: PostgresRepository, communityId: string, houseId?: string) {
  const c = await loadCommunity(repo, communityId); if (c.visibility === 'PRIVATE' && (!houseId || !(await getRole(repo, communityId, houseId)))) throw new Error('Community membership required');
...
export async function decideCommunityMembershipRequest(repo: PostgresRepository, input: { communityId: string; requestId: string; actorHouseId: string; actorHumanId: string; action: 'approve' | 'reject'; rejectionReason?: string }) {
  return repo.transaction(async (tx) => {
    ...
    const request = (await tx.query<Record<string, any>>(`SELECT * FROM community_membership_requests WHERE id=$1 AND community_id=$2 FOR UPDATE`, [input.requestId, input.communityId])).rows[0]; if (!request) throw new Error('Membership request not found'); if (request.status !== 'PENDING') throw new Error('Membership request is already decided');
    const time = await getClock(tx);
...
export async function setCommunityMemberRole(repo: PostgresRepository, input: { communityId: string; actorHouseId: string; actorHumanId: string; targetHouseId: string; role: CommunityRole }) {
  return repo.transaction(async (tx) => {
    ...
    const target = (await tx.query<{ role: CommunityRole; status: string }>(`SELECT role,status FROM community_memberships WHERE community_id=$1 AND house_id=$2 FOR UPDATE`, [input.communityId, input.targetHouseId])).rows[0]; if (!target || target.status !== 'ACTIVE') throw new Error('Target House is not an active member'); if (!['OWNER', 'MODERATOR', 'MEMBER'].includes(input.role)) throw new Error('Invalid community role');
    if (actorRole === 'MODERATOR' && (target.role === 'OWNER' || input.role === 'OWNER')) throw new Error('Moderator cannot manage owners');
    if (target.role === 'OWNER' && input.role !== 'OWNER') { const owners = await tx.query(`SELECT house_id FROM community_memberships WHERE community_id=$1 AND role='OWNER' AND status='ACTIVE' FOR UPDATE`, [input.communityId]); if (owners.rows.length < 2) throw new Error('The last owner cannot be demoted'); }
...
export async function disbandCommunity(repo: PostgresRepository, input: { communityId: string; houseId: string; humanId: string }) {
  return repo.transaction(async (tx) => {
    await assertActor(tx, input.houseId, input.humanId); await loadCommunity(tx, input.communityId, true); if ((await getRole(tx, input.communityId, input.houseId, true)) !== 'OWNER') throw new Error('Community owner permission required'); const time = await getClock(tx);
