...
export function evaluatePlayerObjectives(input: ObjectivesEvaluationInput, rules: Record<string, number> = {}): PlayerObjective[] {
  ...
  const corporationValuation = corpTreasury + 25000;
  const targetCorpVal = rule('objectives.corporation_valuation', 100000);
  const corpProgress = Math.min(100, Math.round((corporationValuation / targetCorpVal) * 100));
  objectives.push({
    ...
    currentValue: corporationValuation,
  ...
  const netWorth = input.netWorth ?? credits + corporationValuation * 0.25;
