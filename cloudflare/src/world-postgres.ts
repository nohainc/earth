...
export async function worldSnapshot(repository: PostgresRepository, viewerId: string): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    world: { health: worldRow.health ?? 0, batch: worldRow.market_batch_seconds ?? 0, livingCostIndex: worldRow.living_cost_index ?? 0, economicStartIndex: startIndex, essentialServicesIndex: worldRow.essential_services_index ?? 0, serviceRatios, serviceStatus, cityQualification, corporationQualification },
