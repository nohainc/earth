...
export async function listSocialTimeline(repo: PostgresRepository, humanId: string, limit = 50) {
  const result = await repo.query(`
    ...
    LEFT JOIN social_initiatives si ON si.id = we.details->>'initiativeId'
...
export async function listSocialRelationships(repo: PostgresRepository, humanId: string) {
  const result = await repo.query(`SELECT r.*, h.display_name, h.dynasty_name FROM social_relationships r JOIN humans h ON h.id = r.other_human_id WHERE r.human_id = $1 ORDER BY r.trust DESC, h.display_name LIMIT 50`, [humanId]);
  return result.rows;
}
...
export async function listSocialDirectory(repo: PostgresRepository, viewerId: string, query = '') {
  ...
  const [humans, businesses, cities, corporations, communities, initiatives] = await Promise.all([
    repo.query(`SELECT h.id, h.display_name, h.dynasty_name, h.standing, h.legacy, m.city_id, ci.id AS city_name FROM humans h LEFT JOIN memberships m ON m.human_id = h.id LEFT JOIN cities ci ON ci.id = m.city_id WHERE h.life_status = 'active' AND h.id <> $1 AND ($2 = '%%' OR h.display_name ILIKE $2 OR h.dynasty_name ILIKE $2) ORDER BY h.standing DESC LIMIT 40`, [viewerId, term]),
