...
export async function listSocialRelationships(repo: PostgresRepository, humanId: string) {
  const result = await repo.query(`SELECT r.*, h.display_name, d.house_name, d.house_name AS dynasty_name FROM social_relationships r JOIN humans h ON h.id = r.other_human_id LEFT JOIN houses d ON d.founder_human_id = h.id WHERE r.human_id = $1 ORDER BY r.trust DESC, h.display_name LIMIT 50`, [humanId]);
  return result.rows;
}
