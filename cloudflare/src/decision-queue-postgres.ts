...
export async function getDecisionQueue(repository: PostgresRepository, houseId: string, limit = 20): Promise<{ decisions: DecisionQueueItem[]; generatedFrom: string; gameDay: number }> {
  const [clock, house, needs, proposals, obligations, research, market, buildings, orders] = await Promise.all([
    ...
    repository.query<{ id: string; title: string; status: string }>(`SELECT id, title, status
       FROM v5_governance_proposals
      WHERE status = 'VOTING'
        AND (subject_type = 'EARTH' OR (subject_type = 'CORPORATION' AND subject_id IN (
          SELECT corporation_id FROM house_affiliations WHERE house_id = $1 AND status = 'ACTIVE'
        )))
      ORDER BY voting_end_game_day, id`, [houseId]),
    ...
    repository.query<{ open_count: string; expiring_count: string }>(`SELECT
         COUNT(*) FILTER (WHERE o.status IN ('OPEN','PARTIAL'))::TEXT AS open_count,
         COUNT(*) FILTER (WHERE o.status IN ('OPEN','PARTIAL')
           AND o.good_til_game_day IS NOT NULL
           AND o.good_til_game_day <= (SELECT game_day + 1 FROM earth_get_current_game_time()))::TEXT AS expiring_count
       FROM market_orders o
       JOIN owner_registry owner ON owner.economic_id = o.owner_economic_id
      WHERE owner.id = $1`, [houseId]),
  ]);
  ...
  const decisions = generateDecisionQueue({
    ...
    house: { has_successor: house.rows[0]?.has_successor === true },
    orders: orders.rows[0] ?? { open_count: '0', expiring_count: '0' },
