...
export async function subscribeToAssetOwnership(repository: PostgresRepository, input: { organizationId: string; assetId: string; investorType: 'HOUSE' | 'ORGANIZATION'; investorId: string; units: string; priceUnits: string; sourceAccountId: string; humanId: string; correlationId: string }): Promise<Record<string, unknown>> {
  return runEconomicMutation(repository, async (tx, clock) => {
    ...
    const posted = await postEconomicTransaction(tx, {
      ...
      sourceType: input.investorType,
      sourceId: input.investorId,
      rulesVersion: 'ownership-v1',
      entries: [
    ...
    return { ok: true, assetId: input.assetId, investorType: input.investorType, investorId: input.investorId, units: units.toString(), amountUnits: amount.toString(), effectiveFromGameDay: day + 1, transactionId: posted.transactionId, correlationId: input.correlationId };
  });
}
...
export async function distributeOwnership(repository: PostgresRepository, input: { organizationId: string; assetId: string; amountUnits: string; humanId: string; correlationId: string }): Promise<Record<string, unknown>> {
  return runEconomicMutation(repository, async (tx, clock) => {
    ...
    const posted = await postEconomicTransaction(tx, {
      correlationId: input.correlationId,
      kind: 'OWNERSHIP_DISTRIBUTION',
      sourceType: 'ORGANIZATION',
      sourceId: input.organizationId,
      rulesVersion: 'ownership-distribution-v1',
      entries: payments.flatMap((payment) => [
        { accountId: source.id, assetId: 1, deltaUnits: (-payment.amount).toString() },
        { accountId: payment.accountId, assetId: 1, deltaUnits: payment.amount.toString() },
      ]),
    }, clock);
    if (!posted.created) return { ok: true, alreadyProcessed: true, correlationId: input.correlationId };
    ...
    for (const payment of payments) await tx.query(`INSERT INTO ownership_distribution_payments (id, distribution_id, holder_type, holder_id, ownership_units, amount_units, economic_transaction_id, correlation_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [`${distributionId}-${payment.holderType}-${payment.holderId}`, distributionId, payment.holderType, payment.holderId, payment.units.toString(), payment.amount.toString(), posted.transactionId, `${input.correlationId}:payment:${payment.holderType}:${payment.holderId}`]);
    ...
    return { ok: true, distributionId, amountUnits: amount.toString(), allocatedUnits: allocated.toString(), retainedRemainderUnits: (amount - allocated).toString(), payments: payments.length, transactionId: posted.transactionId, correlationId: input.correlationId };
  });
}
