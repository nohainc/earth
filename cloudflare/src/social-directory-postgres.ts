...
export async function listSocialDirectory(repo: PostgresRepository, viewerId: string, query = '') {
  ...
  const [humans, businesses, cities, corporations, communities] = await Promise.all([
  ...
  return { humans: humans.rows, businesses: businesses.rows.map(({ owner_id, ...row }) => row), cities: cities.rows, corporations: corporations.rows, communities: communities.rows };
}
