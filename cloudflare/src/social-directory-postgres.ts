...
export async function listSocialDirectory(repo: PostgresRepository, viewerId: string, query = '') {
  ...
  const [humans, cities, corporations, communities] = await Promise.all([
    repo.query(`SELECT h.id, h.display_name, d.house_name, d.house_name AS dynasty_name, h.standing, h.legacy, ha.city_id, ci.id AS city_name FROM humans h LEFT JOIN houses d ON d.id = h.house_id LEFT JOIN house_affiliations ha ON ha.house_id = h.house_id AND ha.status = 'ACTIVE' LEFT JOIN cities ci ON ci.id = ha.city_id WHERE h.life_status = 'active' AND h.id <> $1 AND ($2 = '%%' OR h.display_name ILIKE $2 OR d.house_name ILIKE $2) ORDER BY h.standing DESC LIMIT 40`, [viewerId, term]),
