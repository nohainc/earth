...
export async function handleOrganizationRoutes(request: Request, env: Env, url: URL, viewer: ViewerContext): Promise<Response | null> {
  ...
  if (performanceMatch && request.method === 'GET') {
    const result = await withRepository(env, (repository) => listContractPerformance(repository, performanceMatch[1]));
    if (!result) return Response.json({ ok: false, error: 'PostgreSQL persistence is unavailable' }, { status: 503 });
    return Response.json({ ok: true, ...result, persistence: 'planetscale-postgres' });
  }
  const disputeMatch = url.pathname.match(/^\/api\/organizations\/([^/]+)\/contracts\/performance\/([^/]+)\/dispute$/);
  if (disputeMatch && request.method === 'POST') {
    const parsed = await parseJsonBody<{ reason?: string; correlationId?: string }>(request);
    if (!parsed.ok) return parsed.response;
    const correlationId = resolveIdempotencyKey(request, parsed.value.correlationId);
    if (!correlationId || !parsed.value.reason) return Response.json({ ok: false, error: 'Reason and idempotency key are required' }, { status: 400 });
    try {
      const result = await withRepository(env, (repository) => disputeContractPerformance(repository, { organizationId: disputeMatch[1], performanceId: disputeMatch[2], reason: parsed.value.reason!, humanId: viewer.currentHumanId, correlationId }));
      if (!result) return Response.json({ ok: false, error: 'PostgreSQL persistence is unavailable' }, { status: 503 });
      return Response.json({ ...result, persistence: 'planetscale-postgres' });
    } catch (error) { return errorResponse(error, correlationId, 'Delivery dispute failed.'); }
  }
  const performanceActionMatch = url.pathname.match(/^\/api\/organizations\/([^/]+)\/contracts\/performance\/([^/]+)\/(deliver|accept|reject|resolve)$/);
  if (performanceActionMatch && request.method === 'POST') {
    const parsed = await parseJsonBody<{ note?: string; units?: string; qualityBps?: number; resolution?: 'ACCEPTED' | 'FAILED' | 'WAIVED'; correlationId?: string }>(request);
    if (!parsed.ok) return parsed.response;
    const correlationId = resolveIdempotencyKey(request, parsed.value.correlationId);
    if (!correlationId) return Response.json({ ok: false, error: 'Idempotency key is required' }, { status: 400 });
    try {
      const input = { organizationId: performanceActionMatch[1], performanceId: performanceActionMatch[2], humanId: viewer.currentHumanId, correlationId, note: parsed.value.note, units: parsed.value.units, qualityBps: parsed.value.qualityBps, resolution: parsed.value.resolution };
      const result = await withRepository(env, (repository) => performanceActionMatch[3] === 'deliver' ? submitContractDelivery(repository, input) : performanceActionMatch[3] === 'accept' ? acceptContractDelivery(repository, input) : performanceActionMatch[3] === 'reject' ? rejectContractDelivery(repository, input) : resolveContractDispute(repository, input));
      if (!result) return Response.json({ ok: false, error: 'PostgreSQL persistence is unavailable' }, { status: 503 });
      return Response.json({ ...result, persistence: 'planetscale-postgres' });
    } catch (error) { return errorResponse(error, correlationId, 'Contract performance action failed.'); }
  }
