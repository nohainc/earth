...
export async function listOrganizationTechnologyAdoptions(repository: PostgresRepository, organizationId: string, houseId: string): Promise<Record<string, unknown>> {
  ...
  return { organizationId, adoptions: adoptions.rows, adoptionProposals: proposals.rows, generatedFrom: 'postgres-canonical-facts' };
}
...
export async function proposeTechnologyAdoption(repository: PostgresRepository, input: { organizationId: string; generationId: string; humanId: string; correlationId: string }): Promise<Record<string, unknown>> {
  const generation = (await repository.query<{ name: string; research_points_required: string }>('SELECT name, research_points_required::TEXT FROM technology_generations WHERE id = $1 AND status <> \'RETIRED\'', [input.generationId])).rows[0];
...
export async function adoptTechnologyGeneration(repository: PostgresRepository, input: { organizationId: string; generationId: string; proposalId: string; humanId: string; houseId: string; correlationId: string }): Promise<Record<string, unknown>> {
  return repository.transaction(async (tx) => {
    ...
    const generation = (await tx.query<{ id: string; name: string; research_points_required: string; minimum_game_day: number; predecessor_id: string | null }>('SELECT id, name, research_points_required::TEXT, minimum_game_day, predecessor_id FROM technology_generations WHERE id = $1 AND status <> \'RETIRED\'', [input.generationId])).rows[0];
    if (!generation) throw new Error('Technology generation not found');
    const day = Number((await tx.query<{ game_day: string }>("SELECT game_day::TEXT FROM world_state WHERE id = 'WORLD'")).rows[0]?.game_day ?? 1);
