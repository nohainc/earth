      ...
      if (credential) {
        ...
        if (!recentlySent) {
          try {
            ...
            return Response.json({ ok: false, error: 'The verification email could not be sent. Please try again shortly.' }, { status: 503, headers: { 'x-correlation-id': correlationId } });
          }
        }
      }
    }
